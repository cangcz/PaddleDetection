...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5,
        ...
        # 是否使用某种增强方式
        self.is_addSobel= True  # xzy
        self.is_addNoise = False  # xzy
    ...
    #锐化
    def _addSobel(self, img):
        Sobel_img = cv2.Sobel(img, -1, 1, 0)
        Sobel_img = cv2.add(img, Sobel_img, -1)
        # lap_img = cv2.Laplacian(img, -1)
        # lap_img=color_img+lap_img
        # lap_img = cv2.add(img, lap_img, -1)
        return Sobel_img
...
#####xzy
        ...
        circles1 = cv2.HoughCircles(img2, cv2.HOUGH_GRADIENT, 2, 100, param1=100, param2=100, minRadius=150,
                                    maxRadius=180)
        circles = circles1[0, :, :]  # 提取为二维
        circles = np.uint16(np.around(circles))  # 四舍五入，取整
        # print(circles)
        crop_img = img[circles[0][1] - circles[0][2]:circles[0][1] + circles[0][2],
                   circles[0][0] - circles[0][2]:circles[0][0] + circles[0][2]]
        crop_x_min = circles[0][0] - circles[0][2]
        crop_y_min = circles[0][1] - circles[0][2]
######xzy
        # ---------------------- 裁剪boundingbox ----------------------
        # 裁剪后的boundingbox坐标计算
        crop_bboxes = list()
    ...
    def dataAugment(self, img, bboxes):
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            if self.is_addSobel:
                if random.random() < self.add_sobel_rate:  #锐化
                    change_num += 1
                    img = self._addSobel(img)
